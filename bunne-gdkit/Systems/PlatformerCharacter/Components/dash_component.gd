...
## Temporary override for state stuff, as needed.
...
func update_vector(p_delta: float, p_character: PlatformerCharacter) -> void:
	...
	# Start a dash.
	if dash_enabled and p_character.input_dash and Input.is_action_just_pressed(dash_name):
	...
	# If dashing...
		...
		if end_dash_when_action_released and not p_character.input_dash:
			stop_dash()
		
		# If we've hit a wall, stop dashing.
		elif stop_on_walls and p_character.is_on_wall():
			stop_dash()
		
		# Allow jump-cancelling of dashes.
		elif jump_cancel_dash and p_character.input_jump and Input.is_action_just_pressed(jump_name):
			stop_dash()
