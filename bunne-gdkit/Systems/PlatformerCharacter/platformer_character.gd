...
@export var move_up_name: String = "player_move_up"
...
var input_move_inactive: Vector2 = Vector2.ZERO
var input_jump_inactive: bool = false
var input_dash_inactive: bool = false
...
@export var input_enabled: bool = true :
	set(p_input_enabled):
		...
		if input_enabled:
			load_input_from_inactive()

## Another one that lets you toggle movement entirely.
@export var movement_enabled: bool = true

## Whether analog input is processed with partial strength.
@export var analog_movement: bool = false
...
# So... dumb stuff. Don't actually use ProcessMode.whatever because it breaks
# input processing. I could write some polling specifically for this case,
# but that sounds really stupid, so we'll do it this way instead.
...
func _input(event):
	...
	if event.is_action(move_left_name) or event.is_action(move_right_name):
		input_move_inactive.x = Input.get_axis(move_left_name, move_right_name)
		if not analog_movement:
			input_move_inactive.x = sign(input_move_inactive.x)
	
	elif event.is_action(move_up_name) or event.is_action(move_down_name):
		input_move_inactive.y = Input.get_axis(move_up_name, move_down_name)
		if not analog_movement:
			input_move_inactive.y = sign(input_move_inactive.y)
	
	# Jump
	elif event.is_action(jump_name):
		if event.is_pressed():
			input_jump_inactive = true
		elif event.is_released():
			input_jump_inactive = false
	
	# Dash
	elif event.is_action(dash_name):
		if event.is_pressed():
			input_dash_inactive = true
		elif event.is_released():
			input_dash_inactive = false
...
func load_input_from_inactive():
	input_move = input_move_inactive
	input_jump = input_jump_inactive
	input_dash = input_dash_inactive
...
func update_animation_state():
	...
	elif jump_component != null and jump_component.is_flying:
...
func _physics_process(delta):
	...
	# This is probably really terrible and could stand to be tweaked a LOT.
	#else:
		#if jump_component != null:
			#jump_component.apply_gravity(delta, self)
